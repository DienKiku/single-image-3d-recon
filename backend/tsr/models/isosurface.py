...
try:
    ...
    def marching_cubes(volume: torch.Tensor, thresh: float = 0.0) -> Tuple[torch.FloatTensor, torch.LongTensor]:
        """Drop-in pure Python/C fallback for Marching Cubes using skimage."""
        vol_np = volume.detach().cpu().numpy()
        from skimage.measure import marching_cubes as mc
        try:
            verts, faces, _, _ = mc(vol_np, level=thresh)
